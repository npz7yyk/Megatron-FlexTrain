...
sed "s/CONFIG_BATCH_SIZE/${GLOBAL_BATCH_SIZE}/" ${template_json} \
    ...
    | sed "s/CONFIG_WORLD_SIZE/1/" \
    | sed "s/CONFIG_NUM_LAYERS/${NUM_LAYERS}/" \
    | sed "s/CONFIG_CHECKPOINT_INTERVAL/${CHECKPOINT_INTERVAL}/" \
	  > ${config_json}
...
if [ "${USE_INTERNAL_DATA}" = "true" ]; then
    ...
    VOCAB_PATH=~/gpt-data/gpt2-vocab.json
    MERGE_PATH=~/gpt-data/gpt2-merges.txt
    DATA_BLEND=~/gpt-data/meg-gpt2-oscar-en-10k_text_document
fi
###############################################################################
data_options=" \
...
run_cmd="deepspeed --master_port 6123 ${DIR}/../pretrain_gpt.py ${megatron_options} ${data_options} ${deepspeed_options}"
echo ${run_cmd}
eval ${run_cmd}
set +x
